...
class UInt64 {
  constructor() {
    if (arguments.length === 0) {
      ...
      if (typeof arguments[0] === "number") {
        ...
        const v = UInt64.fromString(arguments[0]);
        this.lo = v.lo;
        this.hi = v.hi;
      } else {
  ...
  times(x) {
    ...
    result.hi = TWO_16 * (a1 * b4 + a2 * b3 + a3 * b2 + a4 * b1) + a2 * b4 + a4 * b2 + a3 * b3;
  ...
  static fromString(x) {
    return x.split('').reduce((acc, ch) => acc.times(10).plus(parseInt(ch)), new UInt64(0));
  }
}
